#!/usr/bin/env python3
"""
Fine-tune EOMT per anomaly detection su Cityscapes + oggetti COCO.
Versione adattata per il progetto su Colab.
"""
...
# Percorsi del progetto (per import)
...
# Costanti
NEW_NUM_CLASSES = 20          # 19 originali + anomalia (classe 19)
IGNORE_INDEX = 255
...
class AnomalyDataset(torch.utils.data.Dataset):
    def __init__(self, img_dir, mask_dir, img_size=(640, 640)):
        ...
        assert len(self.img_paths) == len(self.mask_paths), "Immagini e maschere non corrispondono"
    ...
    def __getitem__(self, idx):
        ...
        for lbl in np.unique(mask_np):
            if lbl == IGNORE_INDEX:
                continue
            masks.append(torch.from_numpy(mask_np == lbl))
...
class AnomalyDataModule(pl.LightningDataModule):
    def __init__(self, train_img_dir, train_mask_dir, val_img_dir, val_mask_dir,
                 img_size=(640,640), batch_size=4, num_workers=4,
    ...
    def setup(self, stage=None):
        self.train_dataset = AnomalyDataset(self.train_img_dir, self.train_mask_dir, self.img_size)
        self.val_dataset = AnomalyDataset(self.val_img_dir, self.val_mask_dir, self.img_size)
    
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True,
                          num_workers=self.num_workers, pin_memory=True,
                          collate_fn=self._train_collate, drop_last=True)
...
def build_model(config_path, ckpt_path, lr=1e-4, load_class_head=False):
    ...
    network = NetCls(masked_attn_enabled=False, num_classes=NEW_NUM_CLASSES, encoder=encoder, **net_kwargs)
    ...
    model_kwargs.update(lr=lr, ckpt_path=None, delta_weights=False, load_ckpt_class_head=load_class_head)
    
    model = LitCls(img_size=img_size, num_classes=NEW_NUM_CLASSES, network=network, **model_kwargs)
    
    # Caricamento pesi
    state = torch.load(ckpt_path, map_location="cpu", weights_only=True)
    if "state_dict" in state:
        state = state["state_dict"]
    # Rimuovi class_head e class_predictor
    filtered = {k:v for k,v in state.items() if not any(x in k for x in ["class_head", "class_predictor"])}
    # Interpola pos_embed se serve
    ...
    if pos_key in filtered:
        ...
        if ckpt_pos.shape != model_pos.shape:
            ...
            interp = F.interpolate(ckpt_2d, size=(model_size, model_size), mode="bicubic", align_corners=False)
            filtered[pos_key] = interp.permute(0,2,3,1).reshape(1, model_size*model_size, dim)
    missing, unexpected = model.load_state_dict(filtered, strict=False)
    print("Missing keys (likely head):", missing)
    print("Unexpected keys:", unexpected)
    # Opzionale: pesa di più la classe anomalia
...
def main():
    ...
    parser.add_argument("--output_dir", default="/content/MLME26/checkpoints")
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        filename="anomaly-{epoch:02d}-{val_iou_all:.3f}",
        monitor="val_iou_all",
        mode="max",
        save_top_k=2,
        save_last=True,
    ...
    logger = TensorBoardLogger(save_dir=args.output_dir, name="eomt_anomaly")
    ...
    print(f"Training completato. Checkpoint salvati in {args.output_dir}")
